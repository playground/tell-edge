...
let build = {
  deploy: () => {
    return new Observable((observer) => {
      console.log(`begin building tello-edge then deploy bundle...`);
      let arg = `ng build --output-hashing=none --configuration production`;
      let child = exec(arg, {maxBuffer: 1024 * 2000}, (err, stdout, stderr) => {
        if(!err) {
          console.log(`done building tello-edge`);
          const filePath = path.resolve(__dirname, '../../tello-express')
          arg = `cp -rf dist ${filePath}`
          exec(arg, {maxBuffer: 1024 * 2000}, (err, stdout, stderr) => {
            if(!err) {
              console.log(`done deploying tello-edge`);
              observer.next();
              observer.complete();
            } else {
